...
function renderQuestions() {
  ...
  questions.forEach((q, idx) => {
    ...
    item.innerHTML = `
      ...
      <div class="review-actions">
        <button class="action-btn comment-btn" data-qid="${idx}">💬 Comments</button>
        <button class="action-btn report-btn" data-qid="${idx}">⚠ Report</button>
      </div>
    `;
  ...
  document.querySelectorAll('.comment-btn').forEach(btn => {
    btn.onclick = () => openComments(Number(btn.dataset.qid));
  });
  document.querySelectorAll('.report-btn').forEach(btn => {
    btn.onclick = () => openReport(Number(btn.dataset.qid));
  });
}
...
async function loadComments() {
  ...
  try {
    const res = await fetch(`${API_BASE}/question/comments/${activeQuestionId}?user_id=${currentUser.user_id}`);
...
function renderComments(comments) {
  ...
  parents.forEach(c => {
    list.appendChild(createCommentEl(c, comments));
    const replies = comments.filter(r => r.parent_comment_id === c.comment_id);
    replies.forEach(r => list.appendChild(createCommentEl(r, comments, true)));
  });
}

function createCommentEl(c, allComments, isReply = false) {
  ...
  el.innerHTML = `
    ...
    <div class="comment-body">
      ...
      <div class="comment-actions">
        ...
        <button class="c-action reply-btn" data-id="${c.comment_id}">Reply</button>
      </div>
    </div>
  `;
...
async function sendComment() {
  ...
  if (!text ||!currentUser.user_id) return;

  try {
    await fetch(`${API_BASE}/question/comment`, {
      method: 'POST',
...
async function submitReport() {
  ...
  if (!reason ||!currentUser.user_id) return;

  try {
    await fetch(`${API_BASE}/question/report`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        question_id: activeQuestionId,
